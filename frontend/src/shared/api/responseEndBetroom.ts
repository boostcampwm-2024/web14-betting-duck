import { z } from "zod";
import { responseBettingRoomInfo } from "./responseBettingRoomInfo";
...
async function responseEndRoom(
  betroomId: string,
  ...
  const bettingRoomInfo = await responseBettingRoomInfo(betroomId);
  if (!bettingRoomInfo?.channel.isAdmin) {
    throw new Error("방장만 베팅 룸을 종료할 수 있습니다.");
  }
}

export { responseEndRoom };
